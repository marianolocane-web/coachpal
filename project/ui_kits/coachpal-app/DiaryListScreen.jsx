function DiaryListScreen({ entries, onOpenEntry, onNewEntry, onSearch }) {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--color-bg-app)', fontFamily: 'var(--font-body)', position: 'relative' }}>
      <div style={{ padding: 'var(--space-6) var(--space-5) 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button
          ...
          style={{
            width: 40, height: 40, borderRadius: '50%', border: '1px solid var(--color-border-default)',
            background: 'var(--color-bg-surface)', cursor: 'pointer', fontSize: 16, color: 'var(--color-text-secondary)',
          }}
        >⌕</button>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 'var(--space-3) var(--space-5) calc(var(--tabbar-height) + var(--space-8))' }}>
        {entries.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 'var(--space-9) var(--space-4)', color: 'var(--color-text-tertiary)', font: 'var(--text-body-md)' }}>
            Todavía no tienes entradas.<br />Empieza por contar cómo ha ido tu día.
          </div>
        ) : (
      ...
      <button
        ...
        style={{
          position: 'absolute', right: 'var(--space-5)', bottom: 'calc(var(--tabbar-height) + var(--space-5))',
          width: 56, height: 56, borderRadius: '50%', border: 'none', cursor: 'pointer',
          background: 'var(--color-brand)', color: 'var(--color-text-on-brand)', fontSize: 26, lineHeight: 1,
